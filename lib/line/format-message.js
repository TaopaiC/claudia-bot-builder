...
class Template extends LineMessage {
  ...
  addMessageButton(label, text) {
    ...
    if (!text || typeof text !== 'string')
      throw new Error('button required a text as a second parameter');
  ...
  addPostbackButton(label, data, text) {
    if (!label || typeof label !== 'string')
      throw new Error('postback button required a label as a first parameter');

    if (label && label.length > 20)
      throw new Error('label can not be more than 20 characters');

    if (!data || typeof data !== 'string')
      throw new Error('postback button required a data as a second parameter');
